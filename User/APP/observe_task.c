...
void 	Observe_task(void)
{
  ...
  //     xvEstimateKF_Update(&vaEstimateKF,INS.MotionAccel_b[1],aver_v);
	...
	// 	chassis_move.x_filter=chassis_move.x_filter+chassis_move.v_filter*((f loat)OBSERVE_TIME/1000.0f);
		
	//如果想直接用轮子速度，不做融合的话可以这样
	 chassis_move.v_filter=(chassis_move.wheel_motor[1].speed-chassis_move.wheel_motor[0].speed)/2.0f;//0.065是轮子半径，电机反馈的是角速度，乘半径后得到线速度，数学模型中定义的是轮子顺时针为正，所以要乘个负号
