...
void Observe_task(void)
{
	...
	while(1)
	{  
		//wr = chassis_move.wheel_motor[1].w_speed - INS.Gyro[0]+right.d_alpha;//右边驱动轮转子相对大地角速度，这里定义的是顺时针为正
		//vrb=wr*0.0675f+right.L0*right.d_theta*arm_cos_f32(right.theta)+right.d_L0*arm_sin_f32(right.theta);//机体b系的速度

		//wl= chassis_move.wheel_motor[0].w_speed + INS.Gyro[0]+left.d_alpha;//左边驱动轮转子相对大地角速度，这里定义的是顺时针为正
		//vlb=wl*0.0675f+left.L0*left.d_theta*arm_cos_f32(left.theta)+left.d_L0*arm_sin_f32(left.theta);//机体b系的速度
		...
		//如果想直接用轮子速度，不做融合的话可以这样（给速度估计取相反极性，原来没有‘-’）
		chassis_move.v_filter=-(chassis_move.wheel_motor[1].speed-chassis_move.wheel_motor[0].speed)/2.0f;
		//chassis_move.v_filter=(chassis_move.wheel_motor[1].speed-chassis_move.wheel_motor[0].speed)/2.0f;（未取反）
		//0.065是轮子半径，电机反馈的是角速度，乘半径后得到线速度，数学模型中定义的是轮子顺时针为正，所以要乘个负号
		chassis_move.x_filter=chassis_move.x_filter+chassis_move.v_filter*((float)OBSERVE_TIME/1000.0f);
		//速度观测
		if(chassis_move.last_leg_set == 1 && chassis_move.recover_flag == 0)
		{
